...
# Columns we actually use for GReaT
REQUIRED_COLUMNS = [
    "provider_specialty",
    "drug_concept_ids",
...
def build_great_input(df: pd.DataFrame) -> pd.DataFrame:
    """
    Build the 3-column GReaT training table from the visit_fact_table.
    ...
    print(f"[INFO] GReaT input table shape: {great_df.shape}")
    return great_df
...
def sample_synthetic(model: GReaT, n_samples: int) -> pd.DataFrame:
    ...
    try:
        ...
        synth_df = pd.DataFrame(columns=REQUIRED_COLUMNS)
...
def save_outputs(
    ...
    """
    Save the cleaned GReaT training input and the synthetic sample.
    ...
    print(f"[INFO] Saved GReaT training input to: {train_path}")
    ...
    print(f"[INFO] Saved synthetic sample to: {synth_path}")
...
def main() -> None:
    ...
    # 3) Build 3-column GReaT input table FROM THE SUBSET
    ...
    print("\n[SUMMARY]")
    print(f"  Real (GReaT input) path        : {train_path}")
    print(f"  Synthetic output path          : {synth_path}")
    ...
    print("  Next step: use the subset files for EHRWalk to ensure a fair comparison.")
